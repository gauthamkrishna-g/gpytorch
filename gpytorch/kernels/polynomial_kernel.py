...
class PolynomialKernel(Kernel):
    ...
    * :math:`c` is an :attr:`offset` parameter.
    ...
    def forward(self, x1, x2, diag=False, last_dim_is_batch=False, **params):
        ...
        if diag:
            return (x1.sum(dim=-1) + self.offset).pow(self.power)

        if x1.dim() == 2 and x2.dim() == 2:
            return torch.addmm(self.offset, x1, x2.transpose(-2, -1)).pow(self.power)
        else:
            return (torch.matmul(x1, x2.transpose(-2, -1)) + self.offset).pow(self.power)
